...
describe('Recusar Orçamento (E2E)', () => {
  ...
  it('deve recusar um orçamento com sucesso quando autenticado como o CLIENTE dono (204)', async () => {
    ...
    const response = await fetch(
      `${baseUrl}/orcamentos/${orcamentoDomain.getId().toValue()}/recusar-orcamento`,
      {
        method: 'PATCH',
        headers: {
    ...
    expect(orcamentoNoBanco?.status).toBe('RECUSADO')
  })

  it('deve retornar 403 (Forbidden) ao tentar recusar orçamento pertencente a outro cliente', async () => {
    // Cliente 1: Dono real do orçamento
    ...
    await prisma.cliente.create({
      data: {
        ...
        documento: cliente1Domain.getDocumento().getValor(),
    ...
    // Cliente 2: Usuário autenticado e cadastrado tentando recusar recurso do Cliente 1
    const { accessToken, usuario } = await makeUsuarioAutenticado(app, {
      role: 'CLIENTE',
    })
    ...
    await prisma.cliente.create({
      data: {
        ...
        documento: cliente2Domain.getDocumento().getValor(),
  ...
  it('deve retornar 403 (AcessoNegadoError) ao tentar recusar como RECEPCAO (role não autorizada)', async () => {
    const { accessToken } = await makeUsuarioAutenticado(app, {
      role: 'RECEPCAO',
    })

    const response = await fetch(
      `${baseUrl}/orcamentos/${randomUUID()}/recusar-orcamento`,
      {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
  ...
  it('deve retornar 404 (Not Found) ao tentar recusar um orçamento inexistente', async () => {
    const { accessToken, usuario } = await makeUsuarioAutenticado(app, {
      role: 'CLIENTE',
    })
  ...
  it('deve retornar 400 (Bad Request) se o ID do orçamento não for um UUID válido', async () => {
    ...
    const response = await fetch(
      `${baseUrl}/orcamentos/id-invalido/recusar-orcamento`,
      {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
