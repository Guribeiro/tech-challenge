...
export type StatusOS = 'RECEBIDA' | 'EM_DIAGNOSTICO' | 'AGUARDANDO_APROVACAO' | 'EM_EXECUCAO' | 'FINALIZADA' | 'ENTREGUE' | 'ENCERRADA_REJEICAO'
...
export class OrdemServico extends Entity<OrdemServicoProps> {
  ...
  public static criar(props: Optional<OrdemServicoProps, 'status'>): OrdemServico {
    ...
    return new OrdemServico(propriedadesCompletas)
  }

  private static validar(props: OrdemServicoProps): void {
    if (!props.clienteId || !props.veiculoId || !props.descricao?.trim()) {
      throw new Error('Cliente, veículo e descrição são obrigatórios para uma ordem de serviço.')
    }

    if (props.servicos && props.servicos.some(item => !item.servico)) {
      throw new Error('Cada serviço solicitado precisa apontar para uma entidade de serviço válida.')
    }

    if (props.itens && props.itens.some(item => !item.descricao?.trim() || item.quantidade <= 0)) {
      throw new Error('Cada item precisa de descrição válida e quantidade maior que zero.')
    }
  }
